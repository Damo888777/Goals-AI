...
async function updateXCodeProj(projPath, widgetBundleId, liveActivityBundleId, developmentTeamId) {
    ...
    xcodeProject.parse(() => {
                ...
                if (liveActivityTargetObj && liveActivityTargetObj.buildPhases) {
                    ...
                    if (liveActivitySourcePhase) {
                        ...
                        for (const liveActivityFile of liveActivitySwiftFiles) {
                            ...
                            // Check if file already exists in project
                            const fileRefs = xcodeProject.hash.project.objects.PBXFileReference;
                            let existingFile = Object.values(fileRefs).find((file) => file && file.path && file.path.includes(fileName));
                            let fileUuid;
                            if (!existingFile) {
                                // Add file reference manually
                                fileUuid = xcodeProject.generateUuid();
                                ...
                                xcodeProject.hash.project.objects.PBXFileReference[fileUuid + '_comment'] = fileName;
                                console.log(`Created file reference for: ${liveActivityFile}`);
                            }
                            else {
                                // Use existing file reference
                                fileUuid = Object.keys(xcodeProject.hash.project.objects.PBXFileReference).find(key => !key.endsWith('_comment') && xcodeProject.hash.project.objects.PBXFileReference[key] === existingFile) || '';
        ...
        // NOTE: This MUST run AFTER main target file addition so file references exist
        if (liveActivityTarget) {
            try {
                const targets = xcodeProject.hash.project.objects.PBXNativeTarget;
                const liveActivityTargetObj = targets[liveActivityTarget.uuid];
                if (liveActivityTargetObj && liveActivityTargetObj.buildPhases) {
                    ...
                    console.warn('Live Activity target object or build phases not found');
                }
            }
            catch (error) {
            ...
            console.warn('Live Activity target not found for LiveActivityModule.swift Target Membership');
        }
